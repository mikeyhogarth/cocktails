...
- **Robot bar integration** — connect to a [CocktailBotHAL](https://github.com/mikeyhogarth/cocktailbot-hal) dispenser to auto-sync your bar inventory and dispense cocktails directly from the app
...
- We use [semantic versioning](https://semver.org/). Please update `CHANGELOG.md` (following [Keep a Changelog](https://keepachangelog.com/en/1.1.0/)) for any user-facing change.
...
- Code formatting is enforced by [Prettier](https://prettier.io/) via a husky pre-commit hook — do not bypass it (`--no-verify`).
